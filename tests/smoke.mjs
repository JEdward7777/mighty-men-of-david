...
const players = [host];
for (const n of names) { players.push(await connect(code, { name: n })); }
check('lobby has 6 players', await waitFor(() => host.state.playerCount === 6));

// 4. Non-host tries to start -> should error
act(players[1], 'start');
check('non-host cannot start', await waitFor(() => players[1].errors.some(e => /host/i.test(e))));

// 5. (Reclaim-by-name behaviour is covered by reclaim-test.mjs.)

// 6. Host starts
act(host, 'start');
check('phase = team_selection', await waitFor(() => host.state.phase === 'team_selection'));
...
// 8. Everyone approves
for (const p of players) { act(p, 'vote', { approve: true }); }
check('phase = vote_result after all vote', await waitFor(() => host.state.phase === 'vote_result'));
check('vote approved', host.state.lastVoteResult.approved === true);

// 9. Host continues -> quest
act(host, 'continueFromVote');
check('phase = quest', await waitFor(() => host.state.phase === 'quest'));
...
for (const p of teamPlayers) { act(p, 'questVote', { success: true }); }
check('phase = quest_result', await waitFor(() => host.state.phase === 'quest_result'));
...
nonTeam.errors = [];
act(nonTeam, 'questVote', { success: true });
// (they're not in quest phase input path) -- just ensure no crash / got an error
check('non-team quest vote errored', await waitFor(() => nonTeam.errors.length > 0));

// 12. Reconnect test: close Bob, reconnect with saved identity
const bob = players[1];
const bobId = bob.identity.playerId, bobToken = bob.identity.token;
bob.ws.terminate();
check('bob marked disconnected',
  await waitFor(() => host.state.players.find(p=>p.id===bobId)?.connected === false));
const bob2 = await connect(code, { playerId: bobId, token: bobToken });
check('bob reconnected same id', bob2.identity.playerId === bobId);
check('bob marked connected again',
  await waitFor(() => host.state.players.find(p=>p.id===bobId)?.connected === true));
